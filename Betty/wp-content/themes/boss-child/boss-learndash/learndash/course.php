						...
						<?php foreach ( $lessons as $lesson ) { ?>
							...
							<div class="lesson post-<?php echo $lesson[ "post" ]->ID; ?> <?php echo $lesson[ "sample" ]; ?> <?php echo (empty( $topics )) ? 'no-topics' : 'has-topics' ?>">
								<h4 class="list-heading">
									...
									<a class="<?php echo ($in_progress && $lesson[ "status" ] == 'notcompleted') ? 'in-progress' : $lesson[ "status" ]; ?>" href="<?php echo $lesson[ "permalink" ] ?>">
            							<span class="list-img">
										 <?php 
										   $img = get_the_post_thumbnail( $lesson[ "post" ]->ID );
										     if ( $img ) : ?>
									           <?php echo $img; ?>
						   				     <?php else : ?>
											  <!-- <img src="<?php echo get_stylesheet_directory_uri(); ?>/images/default-img.jpg" class="default-img" alt="" /> -->
											   <?php endif; ?>
										</span>
										<?php echo $lesson[ "post" ]->post_title; ?></a>
									<p class="lesson-meta">
												...
												<li class="<?php echo $odd_class; ?>">
													<span class="topic_item">
														<a class="<?php echo $completed_class; ?>" href="<?php echo get_permalink( $topic->ID ); ?>" title="<?php echo $topic->post_title; ?>">
															<span class="list-img">
															   <?php 
															     $abc = get_the_post_thumbnail($topic->ID);
														         if ( $abc ) : ?>
														           <?php echo $abc; ?>
											   				     <?php else : ?>
																<!--   <img src="<?php //echo get_stylesheet_directory_uri(); ?>/images/default-img.jpg" class="default-img" alt="" /> -->
